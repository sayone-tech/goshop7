...
#        title1=GeneralSettings.objects.get(site=site)
        for sub in subscribers:
            ...
            email=EmailMessage('[%s]%s'%(site.name,'Shop 7'),content,settings.DEFAULT_FROM_EMAIL,[sub.email],[], headers = {})
    ...
    def save(self, *args, **kwargs):
        ...
        # If send is True and send_status is False, then send newsletter
#        self.send=True
        if (self.send == True):
            try:
