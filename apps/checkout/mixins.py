...
class OrderPlacementMixin(CheckoutSessionMixin):
    ...
    def handle_payment(self, order_number, total, **kwargs):
        ...
        """
        total_amount = "%.2f" % total.incl_tax
        ref = None
        payment_method = self.checkout_session.payment_method()
        if payment_method == 'credit_card':
            source_type, _ = SourceType.objects.get_or_create(code='credit')
            payment_id = self.checkout_session.get_payment_id()
            ref = payment_id
            if not payment_id:
                raise RedirectRequired(reverse('to_cyber'))
