...
class BasketRemoveView(View):
    ...
    #           pop up when product deleted from cart
#            messages.success(request, _('Basket item deleted successfully.'),
#                         extra_tags='safe noicon')
        except:pass
        if self.preview == False and request.is_ajax():
             status='success'
             print "wwww",request.META['HTTP_HOST']
             ctx['html']=render_to_string('oscar/basket/partials/basket_delete.html',{'request':request,},)
...
class WishlistLineDeleteView(View):
    ...
    def post(self,args,**kwargs):
        ...
        if lines:            
            context['status']='wish'
            context['html'] = render_to_string('basket/partials/wishlist_line_delete_popup.html', {'wishs':lines,'request':self.request
                                    
                          })
        else:
            context['status']='no_wish'
            context['html'] = render_to_string('basket/partials/empty_wishlist.html', {'request':self.request
