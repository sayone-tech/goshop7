...
#            self.send_registration_email(user)
    ...
    def send_registration_email(self, user):
        t = loader.get_template('customer/emails/signup.html')
        c = Context({'user': user, 'site': get_current_site(self.request)})
        rendered = t.render(c)
        text_content = strip_tags(rendered)
        html_content = rendered
        email_obj = EmailMultiAlternatives(
            'Thanks for registering', text_content, settings.DEFAULT_FROM_EMAIL, [user.email])
