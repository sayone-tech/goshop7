...
CYBERSOURCE_RESPONSES = {
    ...
    '104': 'The merchantReferenceCode sent with this authorization request\
        matches the merchantReferenceCode of another authorization request that you sent in the last 15 minutes.',
    '150': 'Error: General system failure. ',
    '151': 'Error: The request was received but there was a server timeout.\
        This error does not include timeouts between the client and the server.',
    '152': 'Error: The request was received, but a service did not finish running in time.',
    '201': 'The issuing bank has questions about the request. You do not receive\
        an authorization code in the reply message, but you might receive one\
        verbally by calling the processor.',
    '202': 'Expired card. You might also receive this if the expiration date\
        you provided does not match the date the issuing bank has on file.',
    ...
    '235': 'The requested amount exceeds the originally authorized amount.\
        Occurs, for example, if you try to capture an amount larger than the\
        original authorization amount. This reason code only applies if you\
        are processing a capture through the API.',
    '236': 'Processor Failure',
    '238': 'The authorization has already been captured. This reason code only\
        applies if you are processing a capture through the API.',
    '239': 'The requested transaction amount must match the previous transaction\
        amount. This reason code only applies if you are processing a capture\
        or credit through the API.',
    ...
    '242': 'You requested a capture through the API, but there is no corresponding,\
        ...
        authorization request or if the previously successful authorization has already\
        been used by another capture request. This reason code only applies when\
        you are processing a capture through the API.',
    '243': 'The transaction has already been settled or reversed.',
    '246': 'The capture or credit is not voidable because the capture or credit\
        information has already been submitted to your processor. Or, you\
        requested a void for a type of transaction that cannot be voided. This\
        reason code applies only if you are processing a void through the API.',
    '247': 'You requested a credit for a capture that was previously voided.\
        This reason code applies only if you are processing a void through the API.',
    ...
    def check_response_for_cybersource_error(self):
        if self.response.reasonCode != 100:
            ...
            raise CyberScourceError(
                ...
                CYBERSOURCE_RESPONSES.get(str(self.response.reasonCode),
                                          'Unknown Failure'))
