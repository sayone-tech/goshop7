...
class ToCybersourceView(PaymentDetailsView):
    ...
    def get_context_data(self, **kwargs):
        kwargs['form'] = self.form_class(
            initial={'accountNumber': 4111111111111111, 'firstName': 'Test Name', 'cvNumber': 123})
        return kwargs
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.form.is_valid():
            ...
            url = "https://www.google.com/finance/converter?a=%s&from=%s&to=%s" % \
                (total_incl_tax, 'INR', 'USD')
            ...
            if cyper_transaction.reasonCode == 100:
                ...
                return HttpResponseRedirect(
                    '%s?cyberID=%s&order=%s' % (
                        reverse('cyber_error'), cyper_transaction.id, order_number))
        else:
...
def cyber_error(request):
    ...
    return render_to_response(
        'cybersource/error.html', {'error': error, 'tr': tr}, RequestContext(request))
